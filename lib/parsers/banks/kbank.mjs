...
export const kbank = {
  ...
  parse(allLines) {
    ...
    // เลขสามตัวท้ายของเลขที่บัญชี คือรหัสที่คนที่นี่ใช้เรียกบัญชี และเป็นเลขเดียว
    // กับที่ธนาคารใส่ไว้หน้าชื่อไฟล์ (885resultFile...) — 199-1-33588-5 → 885
    const suffix = accountNo.replace(/\D/g, "").slice(-3);

    const lines = [];
    let openingSatang = null;
    let closingSatang = null;
    // รายละเอียดของแต่ละแถวถูกเก็บเป็นท่อน ๆ ก่อน เพราะ K BIZ ห่อคอลัมน์สุดท้าย
    // ลงบรรทัดถัดไปได้ ตัดตอนอ่านทีละบรรทัดจะได้ข้อความไม่ครบ
    const detailParts = [];
    // แถวล่าสุดที่อ่านสำเร็จ ใช้ตัดสินว่าบรรทัดถัดไปเป็นท่อนที่ห่อลงมาจริงหรือเปล่า
    let wrapTarget = null;
    ...
    allLines.forEach((pageLines, pageIndex) => {
      ...
      pageLines.forEach((line, lineIndex) => {
        ...
        if (!first || !datePattern.test(first.text)) {
          // บรรทัดที่ไม่มีวันที่ อยู่ติดใต้แถวก่อนหน้าพอดี และมีแต่ข้อความในคอลัมน์
          // รายละเอียด คือท่อนหลังของรายละเอียดแถวนั้น — ชื่อ OTA ("BOOKING.C")
          // อยู่ในท่อนนี้เสมอ ทิ้งไปคือทิ้งสิ่งเดียวที่บอกว่าก้อนโอนเป็นของเจ้าไหน
          if (wrapTarget && wrapTarget.lineIndex === lineIndex - 1 && line.runs.length
            && line.runs.every((run) => run.x >= CHANNEL_MAX_X)) {
            detailParts[wrapTarget.index].push(line.runs.map((run) => run.text).join(" "));
            wrapTarget.lineIndex = lineIndex;
          }
          return;
        }
        ...
        detailParts.push([line.runs.filter((run) => run.x >= CHANNEL_MAX_X).map((run) => run.text).join(" ")]);
        wrapTarget = { index: lines.length - 1, lineIndex };
        closingSatang = balanceSatang;
      });
    });

    // ท่อนที่ห่อลงบรรทัดใหม่ถูกตัดกลางคำ ต่อกันตรง ๆ จึงตรงกับต้นฉบับ ส่วน "++"
    // ท้ายสุดคือเครื่องหมายว่าธนาคารตัดข้อความ ไม่ใช่ตัวอักษรของข้อความเอง
    lines.forEach((row, index) => {
      row.detail = detailParts[index].join("").replace(/\+\+?$/, "").trim();
    });
