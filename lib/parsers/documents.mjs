...
export const DOCUMENT_KINDS = {
  ledger: { label: "บัญชีแยกประเภท", accept: ".xlsx", pattern: "*บัญชีแยกประเภท*.xlsx", matches: (name) => name.includes("บัญชีแยกประเภท") && hasExtension(name, ".xlsx") },
  collection: { label: "รายงานการรับเงิน", accept: ".xlsx", pattern: "*รายงานการรับเงิน*.xlsx", matches: (name) => name.includes("รายงานการรับเงิน") && hasExtension(name, ".xlsx") },
  statement885: { label: "Statement บัญชี 885", accept: ".pdf", pattern: "*885*.pdf", matches: (name) => hasAccountCode(name, "885") && hasExtension(name, ".pdf") },
  statement987: { label: "Statement บัญชี 987", accept: ".pdf", pattern: "*987*.pdf", matches: (name) => hasAccountCode(name, "987") && hasExtension(name, ".pdf") },
};
...
export function detectDocumentKind(fileName) {
  const hits = Object.keys(DOCUMENT_KINDS).filter((kind) => DOCUMENT_KINDS[kind].matches(fileName));
  return hits.length === 1 ? hits[0] : null;
}

/** ชื่อไฟล์นี้เข้าได้หลายชนิดหรือเปล่า — ใช้บอกผู้ใช้ว่าทำไมถึงไม่ผ่าน */
export function isAmbiguousDocumentName(fileName) {
  return Object.keys(DOCUMENT_KINDS).filter((kind) => DOCUMENT_KINDS[kind].matches(fileName)).length > 1;
}
