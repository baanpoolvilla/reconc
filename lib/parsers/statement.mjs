...
export function parseStatementBuffer(buffer, sourceName) {
  ...
  if (!headerLines) {
    throw new Error(
      `${sourceName || "ไฟล์นี้"} อ่านข้อความไม่ได้เลย — ต้องเป็น Statement PDF ที่ดาวน์โหลดจาก K BIZ โดยตรง ไม่ใช่ไฟล์สแกนหรือรูปภาพ`,
    );
  }
  ...
  if (!accountNo) {
    throw new Error(
      `${sourceName || "ไฟล์นี้"} ไม่มีเลขที่บัญชีเงินฝากอยู่ในเอกสาร — น่าจะไม่ใช่ Statement ของ K BIZ`,
    );
  }
  if (!lines.length) {
    throw new Error(`${sourceName || "ไฟล์นี้"} ไม่มีรายการเดินบัญชีให้อ่านสักบรรทัด`);
  }
