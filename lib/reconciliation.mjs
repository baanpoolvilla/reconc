 ...
 */
export function reconcileAccount(statement, receipts, bookings) {
  const groups = [];
  const exceptions = [];
  ...
  for (const date of dates) {
    for (const line of availableLines(date)) {
      const subset = findExactSubset(availableReceipts(date), line.amountSatang);
  ...
  for (const date of dates) {
    for (const receipt of availableReceipts(date)) {
      const subset = findExactSubset(availableLines(date), receipt.amountSatang);
      if (!subset || subset.length < 2) continue;
      takeReceipts([receipt]);
      takeLines(subset);
...
export function reconcile(dataset) {
  ...
  for (const statement of dataset.statements) {
    ...
    const { groups, exceptions } = reconcileAccount(statement, receipts, bookings);
