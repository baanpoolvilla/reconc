...
// Reconciliation engine — ruleset v3.1.0
//
...
export const RULESET_VERSION = "3.1.0";
 ...
 *
 * งวดที่ใช้ผูกคือเดือนของ "วันที่สร้างคำจอง" เพราะนั่นคือวันเดียวที่กฎ R01 มองหา
 * ในฝั่ง statement รายการที่ไม่มีคำจองรองรับใช้เดือนของวันที่รับเงินแทน
 *
...
function assignToStatements(receipts, statements, bookings) {
  ...
  for (const receipt of receipts) {
    ...
    const period = periodOf(bookings.get(receipt.reservationNo)?.createdDate || receipt.date);
