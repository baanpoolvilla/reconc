...
// v3.3.0 · งวดที่เหลือวัน Check-in (R08)
//
// คำจองหนึ่งใบจ่ายได้หลายงวด และสองงวดนั้นเข้าบัญชีคนละวันตามธรรมชาติของมัน:
// มัดจำเข้าวันที่สร้างคำจอง ส่วนค่าห้องที่เหลือเข้าวันที่แขก Check-in
//
// กฎ R01 อธิบายงวดแรกได้ครบ และไม่เคยอธิบายงวดที่สองได้เลย เพราะวันที่มันมองหา
// คือวันที่สร้างคำจอง ซึ่งผ่านไปแล้วตอนแขกมาถึง ใบงวดที่สองจึงตกเป็นข้อยกเว้น
// "ไม่มีเงินเข้าในวันที่สร้างคำจอง" ทั้งที่เงินเข้าบัญชีตรงวันและตรงยอด
//
// R08 จับงวดนั้น ด้วยหลักฐานความแน่นเท่ากับ R03: วันเดียวกันและยอดตรงพอดี
// ต่างกันแค่ "วันเดียวกัน" นับจากวัน Check-in ซึ่งเป็นวันที่เงินก้อนนั้นเคลื่อนจริง
// R01 ไม่ถูกผ่อนแม้แต่น้อย — มันยังคุมงวดแรกอยู่เหมือนเดิมทุกประการ
...
export const RULESET_VERSION = "3.3.0";
...
export const RULES = [
  ...
  { id: "R08", label: "งวดที่เหลือวัน Check-in", detail: "ยอดตรงพอดี เมื่อรับเงินและเงินเข้าตรงกับวัน Check-in — คือค่าห้องส่วนที่เหลือของคำจองที่จ่ายมัดจำไว้แล้ว", score: 94 },
...
function matchOptions(options) {
  ...
  return {
    ...
    heldLineIds: new Set(merged.heldLineIds ?? []),
  };
}
...
export function reconcileAccount(statement, receipts, bookings, options) {
  ...
  // R08 · งวดที่เหลือวัน Check-in
  //
  // คำจองหนึ่งใบจ่ายเป็นงวดได้ และงวดที่สองเข้าบัญชีวันที่แขก Check-in ไม่ใช่วันที่
  // สร้างคำจอง กฎ R01 จึงมองไม่เห็นมันตลอดกาล — ไม่ใช่เพราะข้อมูลผิด แต่เพราะวันที่
  // มันมองหาผ่านไปแล้วตั้งแต่ตอนจอง
  //
  // หลักฐานของกฎนี้แน่นเท่า R03 ทุกประการ: วันเดียวกันและยอดตรงพอดี ไม่มี tolerance
  // ต่างกันแค่ว่า "วันเดียวกัน" นับจากวัน Check-in ซึ่งเป็นวันที่เงินก้อนนี้เคลื่อนจริง
  //
  // เงื่อนไข receipt.date === receipt.checkIn คือสิ่งที่ทำให้กฎนี้ไม่กลายเป็น
  // "จับคู่ด้วยยอดวันไหนก็ได้" — ใบที่รับเงินคนละวันกับวัน Check-in ไม่เข้าข่าย
  // และยังตกเป็นข้อยกเว้นให้คนดูเหมือนเดิม
  {
    ...
    for (const checkIn of [...onCheckIn.keys()].sort()) {
      for (const receipt of onCheckIn.get(checkIn)) {
        ...
        // วันที่สร้างคำจองตรงกับวัน Check-in อยู่แล้ว = R03 มีสิทธิ์ก่อน และได้จับ
        // ไปแล้วถ้าจับได้ กฎนี้จึงเหลือไว้เฉพาะงวดที่ R01 อธิบายไม่ได้จริง ๆ
        const line = availableLines(checkIn).find((candidate) => candidate.amountSatang === receipt.amountSatang);
        if (!line) continue;
        takeReceipts([receipt]);
        takeLines([line]);
        groups.push(makeGroup("1:1+CHECKIN", 94, checkIn, statement, [receipt], [line], null, { matchedOn: "checkIn" }));
      }
    }
  }
  ...
  for (const receipt of openReceipts.filter((item) => !usedReceipts.has(item.id))) {
    // วันที่กฎมองหาเงินเข้าให้ใบนี้ — งวดแรกคือวันที่สร้างคำจอง (R01) ส่วนใบที่
    // รับเงินวัน Check-in มีอีกวันที่ต้องดูด้วย (R08)
    //
    // ถ้ารายงานเฉพาะวันที่สร้างคำจอง ใบงวดที่เหลือที่ยอดเพี้ยนไปสตางค์เดียวจะขึ้นว่า
    // "ไม่มีเงินเข้าในวันที่สร้างคำจอง" ซึ่งชี้ผู้ตรวจไปผิดวันทั้งวัน ทั้งที่เงินก้อน
    // ที่ใช่นอนรออยู่ในวัน Check-in ห่างจากยอดแค่นิดเดียว
    const paidOnCheckIn = Boolean(receipt.checkIn) && receipt.date === receipt.checkIn;
    const lookupDates = paidOnCheckIn && receipt.checkIn !== receipt.matchDate
      ? [receipt.matchDate, receipt.checkIn]
      : [receipt.matchDate];
    const sameDayLines = lookupDates.flatMap((date) => linesByDate.get(date) ?? []);
...
function makeGroup(type, score, date, statement, receipts, lines, decision = null, extra = {}) {
  ...
  return {
    ...
    rulesPassed: extra.matchedOn === "checkIn"
      ? [
        {
          id: "R08",
...
function assignToStatements(receipts, statements, bookings) {
  ...
  for (const receipt of receipts) {
    ...
    // งวดที่เหลือของคำจองเดือนก่อน เข้าบัญชีในเดือนที่แขก Check-in ไม่ใช่เดือนที่จอง
    // การถอยไปหา statement ของเดือนที่รับเงินจริง จึงเป็นทางเดียวที่ใบงวดที่สอง
    // ของคำจองข้ามเดือนจะมี statement ให้เทียบ แทนที่จะถูกรายงานว่าไม่มี Statement
    const paid = periodOf(receipt.date);
    const statement = byKey.get(`${method}|${period}`)
      ?? byKey.get(`${method}|${booked}`)
      ?? byKey.get(`${method}|${paid}`);
...
export function reconcile(dataset, options) {
  ...
  for (const statement of statements) {
    ...
    // เงินเข้าที่พักไว้ถูกกันออกตั้งแต่ต้น จึงไม่กลายเป็นข้อยกเว้น ไม่ถูกจับคู่
    // และไม่ถูกเสนอเข้าก้อนโอน — แต่ยังอยู่ในยอดคุมของ statement เหมือนเดิม
    const skip = new Set([...usedLines, ...opts.heldLineIds]);
    const { groups, exceptions } = reconcileAccount(statement, open, bookings, { ...opts, excludeLineIds: skip });
  ...
  return {
    ...
    summary: {
      ...
      // เงินเข้าที่พักไว้ ไม่ใช่งานที่เสร็จ และไม่ใช่งานที่ค้าง — เป็นหมวดของตัวเอง
      heldLines: opts.heldLineIds.size,
    },
  };
}
