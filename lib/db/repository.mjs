...
async function loadSources(db) {
  ...
  const rows = await db.query(
    "SELECT kind, name, row_count, period, period_start, period_end FROM clearclose.documents ORDER BY period DESC",
  );
  const kindNames = {
    ledger: "ledger", collection: "collection_report",
    statement885: "bank_statement_885", statement987: "bank_statement_987",
  };
  return [...rows]
    .sort((a, b) => String(b.period).localeCompare(String(a.period)) || order.indexOf(a.kind) - order.indexOf(b.kind))
    .map((row) => ({
      kind: kindNames[row.kind] ?? row.kind,
      label: DOCUMENT_KINDS[row.kind]?.label ?? row.kind,
      name: row.name,
