...
import { DEFAULT_ORGANIZATION, normalizeOrganization } from "./issued-receipts.mjs";
...
export const DEFAULT_SETTINGS = {
  ...
  accounts: [],
  // ผู้ออกใบเสร็จรับเงิน — ว่างไว้โดยตั้งใจ ระบบไม่รู้จักนิติบุคคลของใครล่วงหน้า
  // และเอกสารที่ส่งออกไปหาคนนอกไม่ควรมีตัวอย่างค่าตั้งต้นหลุดไปด้วย
...
export function normalizeSettings(raw) {
  ...
  return {
    ...
    organization: normalizeOrganization(source.organization),
    display: {
 ...
 */
export function applySettings(dataset, settings, decisions = [], holds = []) {
  const heldLineIds = holds.map((item) => item.bankLineId ?? item);
  const bookingIndex = new Map(dataset.bookings.map((booking) => [booking.reservationNo, booking]));
  const kept = [];
  const excluded = [];
  ...
  const reconciliation = dataset.meta.sources.length
    ? reconcile(core, { ...settings.matching, securityDepositSatang: effectiveDepositSatang(settings), decisions, heldLineIds })
  ...
  return {
    ...
    settlements: dataset.meta.sources.length
      ? proposeSettlements(effective, reconciliation, {
        ...settings.settlement,
        heldLineIds,
        // งวดที่ระบบมีรายงานการรับเงินอยู่จริง ใช้ตอบว่าก้อนที่หาคำจองไม่พบนั้น
        // "ไม่มีอยู่จริง" หรือแค่ "ยังไม่ได้อัปโหลดเดือนนั้น"
        knownPeriods: dataset.meta.periods ?? [],
        // ใบที่ถูกตัดออกด้วยการตั้งค่า ส่งไปให้ข้อเสนอบอกได้ว่ามีของที่เข้าเงื่อนไข
        // อยู่แต่ถูกกันไว้ — ไม่ได้ถูกนำมาจับคู่ แค่ใช้ตอบว่าทำไมถึงหาไม่เจอ
        excludedReceipts: excluded,
      })
      : [],
    // เงินเข้าที่พักไว้ พร้อมยอดรวม — ไม่ใช่งานที่เสร็จ และไม่ใช่งานที่ค้าง
    holds,
    heldSatang: holds.reduce((sum, item) => sum + (item.amountSatang ?? 0), 0),
...
export function scopeToPeriod(effective, period) {
  ...
  const holds = effective.holds.filter((item) => periodOf(item.date) === period);
