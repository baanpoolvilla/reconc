 ...
 */
export function applySettings(dataset, settings) {
  const bookingIndex = new Map(dataset.bookings.map((booking) => [booking.reservationNo, booking]));
  const kept = [];
  const excluded = [];
  ...
  const reconciliation = dataset.meta.sources.length ? reconcile(core, settings.matching) : dataset.reconciliation;
  ...
  return {
    dataset: { ...dataset, receipts: kept, reconciliation },
    excluded,
