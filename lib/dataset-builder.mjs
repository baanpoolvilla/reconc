...
/**
 * The dataset, from documents that have just been parsed rather than stored.
 *
 * The upload endpoint writes rows to Postgres and reconciles from there, so this
 * is not on the production path; it exists so a parser can be handed straight to
 * the engine in a test without a database standing in between.
 *
...
export function buildDataset(documents) {
  return assembleDataset({
    ...
    statements: documents.filter((document) => document.statement).map((document) => withPeriod(document.statement)),
