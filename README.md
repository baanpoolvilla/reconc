...
lib/settings.ts              type ให้ settings-core และการจำค่าใน localStorage
...
app/settings-view.tsx        หน้าการตั้งค่า
app/ui.tsx                   primitive ที่ใช้ร่วมกันทุกหน้า
...
แต่ละหน้าเปิดตรงได้ด้วย hash — `#overview` `#ledger` `#matching` `#exceptions` `#bookings` `#statements` `#upload` `#rules` `#settings`

| หน้า | ใช้ทำอะไร |
|---|---|
| ภาพรวม | อัตราการจับคู่ · control total ของแต่ละบัญชี · สาเหตุที่จับคู่ไม่ได้ · ช่องทางนอกขอบเขต |
| ตารางรับเงิน | ตารางแบบ Excel ยึดรายการรับเงินเป็นแถว — วันจอง รหัสบ้าน ผู้จอง ช่องทางติดต่อ ยอดรับ ยอดจองรวม จ่ายจริง คงค้าง · ยอดที่กระทบตรงขึ้นกรอบเขียว ไม่ตรงขึ้นกรอบแดง · ส่งออก CSV ได้ |
...
| การตั้งค่า | ตัวกรองรายการที่ไม่นำเข้ากระทบยอด · รูปแบบการจับคู่ที่อนุญาต · การแสดงผล · ข้อมูลระบบ · ส่งออก/นำเข้าไฟล์ตั้งค่า |

## การตั้งค่า

หน้า **การตั้งค่า** (`#settings`) เป็นชั้นเดียวที่วางทับเอกสารต้นทาง
ทำได้อย่างเดียวคือ **ไม่นำบางรายการเข้ากระทบยอด** แล้วสั่งให้ engine คำนวณใหม่ทั้งรอบ
ไฟล์ใน `data/` และข้อมูลในฐานข้อมูล**ไม่เคยถูกแก้ไข** และรายการที่ถูกตัดออกยังเรียกดูได้ครบ
พร้อมเหตุผลว่าถูกตัดด้วยกฎข้อไหน
...
- **รูปแบบการจับคู่** — เปิด/ปิด `N:1` และ `1:N` และกำหนดจำนวนรายการสูงสุดต่อกลุ่ม (2–6)
  ส่วน **R01 วันที่** และ **R02 ยอดตรงพอดี** ล็อกไว้เสมอ แก้ไขไม่ได้
- **การแสดงผล** — จำนวนแถวสูงสุดในตาราง และจะแสดงรายการที่ถูกตัดออกในตารางด้วยหรือไม่

ค่าที่ตั้งไว้เก็บใน `localStorage` ของเบราว์เซอร์เครื่องนั้น ไม่ถูกส่งขึ้นเซิร์ฟเวอร์
ต้องการให้เครื่องอื่นเหมือนกันให้ **ส่งออกไฟล์ JSON** แล้วนำเข้าที่เครื่องนั้น
